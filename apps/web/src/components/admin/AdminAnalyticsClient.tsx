...
import type { AdminAnalyticsClientProps } from "@/types/admin-analytics";
import type {
...
function analyticsHref(filters: AdminReportFilters, range: AdminReportRange) {
  if (range !== "custom") return `/admin/analytics?range=${range}`;
...
export default function AdminAnalyticsClient({ result }: AdminAnalyticsClientProps) {
  ...
  return (
    <>
      <AdminFilters
        ...
        filters={[
          ...rangeOptions.map((range) => ({
            label: range.label,
            href: analyticsHref(result.filters, range.value),
            active: result.filters.range === range.value,
          })),
          {
            label: "Custom",
            href: analyticsHref(result.filters, "custom"),
      ...
      <div className="grid gap-5 xl:grid-cols-4">
        ...
        <DistributionPanel title="Payment status distribution" items={result.paymentStatusDistribution} />
      ...
      <ChartCard
        ...
        caption="Daily platform occupancy percentage"
